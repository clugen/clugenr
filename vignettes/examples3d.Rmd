---
title: "Examples in 3D"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Examples in 3D}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  out.width = "100%",
  fig.asp=0.4,
  fig.width=8
)
```

This vignette contains a number of examples on how to use `clugenr` in 3D.
Examples require the following setup code:

```{r setup, message=FALSE, warning=FALSE}
library(clugenr)    # The clugenr library
options(rgl.useNULL = TRUE)  # Create RGL plots in systems without displays (CI)
library(rgl)
setupKnitr(autoprint = TRUE) # Render RGL plots directly on generated page

# Load helper functions for plotting examples
source("plot_examples.R", local = knitr::knit_global())

# Keep examples reproducible in newer R versions 
RNGversion("3.6.0") 
```

The
[`plot_examples.R`](https://github.com/clugen/clugenr/blob/master/vignettes/plot_examples.R)
containing the helper functions for plotting these examples is available
[here](https://github.com/clugen/clugenr/blob/master/vignettes/plot_examples.R).

## Manipulating the direction of cluster-supporting lines

### Using the `direction` parameter

```{r}
seed <- 123
```

```{r}
set.seed(seed)
e40 <- clugen(3, 4, 500, c(1, 0, 0), 0, c(10, 10, 10), 15, 1.5, 0.5)
set.seed(seed)
e41 <- clugen(3, 4, 500, c(1, 1, 1), 0, c(10, 10, 10), 15, 1.5, 0.5)
set.seed(seed)
e42 <- clugen(3, 4, 500, c(0, 0, 1), 0, c(10, 10, 10), 15, 1.5, 0.5)
```

```{r}
plot_examples_3d(list(e = e40, t = "e40: direction = [1, 0, 0]"),
                 list(e = e41, t = "e41: direction = [1, 1, 1]"),
                 list(e = e42, t = "e42: direction = [0, 0, 1]"))
```

### Changing the `angle_disp` parameter and using a custom `angle_deltas_fn` function

```{r}
seed <- 123
```

```{r}
# Custom angle_deltas function: arbitrarily rotate some clusters by 90 degrees
angdel_90 <- function(nclu, astd) sample(c(0, pi / 2), nclu, replace = T)
```

```{r}
set.seed(seed)
e43 <- clugen(3, 6, 1000, c(1, 0, 0), 0, c(10, 10, 10), 15, 1.5, 0.5)
set.seed(seed)
e44 <- clugen(3, 6, 1000, c(1, 0, 0), pi / 8, c(10, 10, 10), 15, 1.5, 0.5)
set.seed(seed)
e45 <- clugen(3, 6, 1000, c(1, 0, 0), 0, c(10, 10, 10), 15, 1.5, 0.5,
              angle_deltas_fn = angdel_90)
```

```{r}
plot_examples_3d(list(e = e43, t = "e43: angle_disp = 0"),
                 list(e = e44, t = "e44: angle_disp = π / 8"),
                 list(e = e45, t = "e45: custom angle_deltas function"))
```


## Manipulating the length of cluster-supporting lines

```{r}
seed <- 123
```

### Using the `llength` parameter

```{r}
set.seed(seed)
e46 <- clugen(3, 5, 800, c(1, 0, 0), pi / 10, c(10, 10, 10), 0, 0, 0.5,
              point_dist_fn = "n")
set.seed(seed)
e47 <- clugen(3, 5, 800, c(1, 0, 0), pi / 10, c(10, 10, 10), 10, 0, 0.5,
              point_dist_fn = "n")
set.seed(seed)
e48 <- clugen(3, 5, 800, c(1, 0, 0), pi / 10, c(10, 10, 10), 30, 0, 0.5,
              point_dist_fn = "n")
```

```{r}
plot_examples_3d(list(e = e46, t = "e46: llength = 0"),
                 list(e = e47, t = "e47: llength = 10"),
                 list(e = e48, t = "e48: llength = 30"))
```

### Changing the `llength_disp` parameter and using a custom `llengths_fn` function

```{r}
# Custom llengths function: line lengths tend to grow for each new cluster
llen_grow <- function(nclu, llen, llenstd) llen * (0:(nclu - 1) + rnorm(nclu, sd = llenstd))
```

```{r}
set.seed(seed)
e49 <- clugen(3, 5, 800, c(1, 0, 0), pi / 10, c(10, 10, 10), 15,  0.0, 0.5,
              point_dist_fn = "n")
set.seed(seed)
e50 <- clugen(3, 5, 800, c(1, 0, 0), pi / 10, c(10, 10, 10), 15, 10.0, 0.5,
              point_dist_fn = "n")
set.seed(seed)
e51 <- clugen(3, 5, 800, c(1, 0, 0), pi / 10, c(10, 10, 10), 10,  0.1, 0.5,
              point_dist_fn = "n",
              llengths_fn = llen_grow)
```

```{r}
plot_examples_3d(list(e = e49, t = "e49: llength_disp = 0.0"),
                 list(e = e50, t = "e50: llength_disp = 10.0"),
                 list(e = e51, t = "e51: custom llengths function"))
```

## Manipulating relative cluster positions

### Using the `cluster_sep` parameter

```{r}
seed <- 321
```

```{r}
set.seed(seed)
e52 <- clugen(3, 8, 1000, c(1, 1, 1), pi / 4, c(30, 10, 10), 25, 4, 3)
set.seed(seed)
e53 <- clugen(3, 8, 1000, c(1, 1, 1), pi / 4, c(10, 30, 10), 25, 4, 3)
set.seed(seed)
e54 <- clugen(3, 8, 1000, c(1, 1, 1), pi / 4, c(10, 10, 30), 25, 4, 3)
```

```{r}
plot_examples_3d(list(e = e52, t = "e52: cluster_sep = [30, 10, 10]"),
                 list(e = e53, t = "e53: cluster_sep = [10, 30, 10]"),
                 list(e = e54, t = "e54: cluster_sep = [10, 10, 30]"))
```

### Changing the `cluster_offset` parameter and using a custom `clucenters_fn` function

```{r}
seed <- 321
```

```{r}
# Custom clucenters function: places clusters in a diagonal
centers_diag <- function(nclu, csep, coff) {
  matrix(1, nrow = nclu, ncol = length(csep)) * (1:nclu * max(csep)) + rep(coff, each = nclu)
}
```

```{r}
set.seed(seed)
e55 <- clugen(3, 8, 1000, c(1, 1, 1), pi / 4, c(10, 10, 10), 12, 3, 2.5)
set.seed(seed)
e56 <- clugen(3, 8, 1000, c(1, 1, 1), pi / 4, c(10, 10, 10), 12, 3, 2.5,
              cluster_offset = c(20, -20, 20))
set.seed(seed)
e57 <- clugen(3, 8, 1000, c(1, 1, 1), pi / 4, c(10, 10, 10), 12, 3, 2.5,
              cluster_offset = c(-50, -50, -50), clucenters_fn = centers_diag)
```

```{r}
plot_examples_3d(list(e = e55, t = "e55: default"),
                 list(e = e56, t = "e56: cluster_offset = [20, -20, 20]"),
                 list(e = e57, t = "e57: custom clucenters function"))
```

## Lateral dispersion and placement of point projections on the line

```{r}
seed <- 456
```

### Normal projection placement (default): `proj_dist_fn = "norm"`

```{r}
set.seed(seed)
e58 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 0.0)
set.seed(seed)
e59 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 1.0)
set.seed(seed)
e60 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 3.0)
```

```{r}
plot_examples_3d(list(e = e58, t = "e58: lateral_disp = 0"),
                 list(e = e59, t = "e59: lateral_disp = 1"),
                 list(e = e60, t = "e60: lateral_disp = 3"))
```

### Uniform projection placement: `proj_dist_fn = "unif"`

```{r}
set.seed(seed)
e61 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 0.0,
              proj_dist_fn = "unif")
set.seed(seed)
e62 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 1.0,
              proj_dist_fn = "unif")
set.seed(seed)
e63 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 3.0,
              proj_dist_fn = "unif")
```

```{r}
plot_examples_3d(list(e = e61, t = "e61: lateral_disp = 0"),
                 list(e = e62, t = "e62: lateral_disp = 1"),
                 list(e = e63, t = "e63: lateral_disp = 3"))
```

### Custom projection placement using the Beta distribution

```{r}
# Custom proj_dist_fn: point projections placed using the Beta distribution
proj_beta <- function(len, n) len * rbeta(n, 0.1, 0.1) - len / 2
```

```{r}
set.seed(seed)
e64 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 0.0, proj_dist_fn = proj_beta)
set.seed(seed)
e65 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 1.0, proj_dist_fn = proj_beta)
set.seed(seed)
e66 <- clugen(3, 4, 1000, c(1, 0, 0), pi / 2, c(20, 20, 20), 13, 2, 3.0, proj_dist_fn = proj_beta)
```

```{r}
plot_examples_3d(list(e = e64, t = "e64: lateral_disp = 0"),
                 list(e = e65, t = "e65: lateral_disp = 1"),
                 list(e = e66, t = "e66: lateral_disp = 3"))
```

## Controlling final point positions from their projections on the cluster-supporting line

```{r}
seed <- 12321
```

```{r}
# Custom proj_dist_fn: point projections placed using the Beta distribution
proj_beta <- function(len, n) len * rbeta(n, 0.1, 0.1) - len / 2
```

### Points on hyperplane orthogonal to cluster-supporting line (default): `point_dist_fn = "n-1"`

```{r}
set.seed(seed)
e67 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2)
set.seed(seed)
e68 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
             proj_dist_fn = "unif")
set.seed(seed)
e69 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
             proj_dist_fn = proj_beta)
```

```{r}
plot_examples_3d(list(e = e67, t = "e67: proj_dist_fn = 'norm' (default)"),
                 list(e = e68, t = "e68: proj_dist_fn = 'unif'"),
                 list(e = e69, t = "e69: custom proj_dist_fn (Beta dist.)"))
```

### Points around projection on cluster-supporting line: `point_dist_fn = "n"`

```{r}
set.seed(seed)
e70 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
              point_dist_fn = "n")
set.seed(seed)
e71 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
              point_dist_fn = "n", proj_dist_fn = "unif")
set.seed(seed)
e72 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
              point_dist_fn = "n", proj_dist_fn = proj_beta)
```

```{r}
plot_examples_3d(list(e = e70, t = "e70: proj_dist_fn = 'norm' (default)"),
                 list(e = e71, t = "e71: proj_dist_fn = 'unif'"),
                 list(e = e72, t = "e72: custom proj_dist_fn (Beta dist.)"))
```

### Custom point placement using the exponential distribution

```{r}
# Custom point_dist_fn: final points placed using the Exponential distribution
clupoints_n_1_exp <- function(projs, lat_std, len, clu_dir, clu_ctr) {
    dist_exp <- function(npts, lstd) lstd * rexp(npts, rate = 2 / lstd)
    clupoints_n_1_template(projs, lat_std, clu_dir, dist_exp)
}
```

```{r}
set.seed(seed)
e73 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
              point_dist_fn = clupoints_n_1_exp)
set.seed(seed)
e74 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
              point_dist_fn = clupoints_n_1_exp, proj_dist_fn = "unif")
set.seed(seed)
e75 <- clugen(3, 5, 1500, c(1, 0, 0), pi / 3, c(20, 20, 20), 22, 3, 2,
              point_dist_fn = clupoints_n_1_exp, proj_dist_fn = proj_beta)
```

```{r}
plot_examples_3d(list(e = e73, t = "e73: proj_dist_fn = 'norm' (default)"),
                 list(e = e74, t = "e74: proj_dist_fn = 'unif'"),
                 list(e = e75, t = "e75: custom proj_dist_fn (Beta dist.)"))
```

### Manipulating cluster sizes

```{r}
seed <- 87
```

```{r}
# Custom clucenters_fn (all): yields fixed positions for the clusters
centers_fixed <- function(nclu, csep, coff) {
  matrix(c(-csep[1], -csep[2], -csep[3], csep[1], -csep[2], -csep[3],
           -csep[1], csep[2], csep[3], csep[1], csep[2], csep[3]),
         nrow = nclu, byrow = T)
}

# Custom clusizes_fn (e77): cluster sizes determined via the uniform distribution,
# no correction for total points
clusizes_unif <- function(nclu, npts, ae) sample(2 * npts / nclu, nclu, replace = T)

# Custom clusizes_fn (e78): clusters all have the same size, no correction for
# total points
clusizes_equal <- function(nclu, npts, ae) npts %/% nclu * rep.int(1, nclu)
```

```{r}
set.seed(seed)
e76 <- clugen(3, 4, 1500, c(1, 1, 1), pi, c(20, 20, 20), 0, 0, 5,
              point_dist_fn = "n",
              clucenters_fn = centers_fixed)
set.seed(seed)
e77 <- clugen(3, 4, 1500, c(1, 1, 1), pi, c(20, 20, 20), 0, 0, 5,
              clusizes_fn = clusizes_unif, point_dist_fn = "n",
              clucenters_fn = centers_fixed)
set.seed(seed)
e78 <- clugen(3, 4, 1500, c(1, 1, 1), pi, c(20, 20, 20), 0, 0, 5,
              clusizes_fn = clusizes_equal, point_dist_fn = "n",
              clucenters_fn = centers_fixed)
```

```{r}
plot_examples_3d(list(e = e76, t = "e76: normal dist. (default)"),
                 list(e = e77, t = "e77: unif. dist. (custom)"),
                 list(e = e78, t = "e78: equal size (custom)"))
```
